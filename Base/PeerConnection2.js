...
export class PeerConnection {
    ...
    constructor(params={stunServers:[], timeout:14, maxRetransmits:null, ordered:true}) { // timeout must be superior to HTTP RDV timeout and ICE connection timeout (the both are around 10 sec)
        ...
        this._qrtTimer = Timer.Set(() => this.manage(), 1000); // TODO remove timer and call explicitly the manage!
        this._qrt = new QRT();
        this._qrt.onFlush = () => this.onFlush();
        this._qrt.onSend = (packet) => {
            try {
        ...
        this._qrt.onRecv = (packet) => {
            this._recvTime = Util.Time();
            this._recvByteRate.addBytes(packet.byteLength);
            this.onData(packet);
        }
        ...
        this._connection.ondatachannel = (e) =>  {
            ...
            this._channel.onmessage = (e) => this._qrt.recv(new Uint8Array(e.data));
        ...
        this._connection.oniceconnectionstatechange = (e) => {
            // console.log(this._connection.iceConnectionState);
            if(this._timeoutID)
    ...
    manage() { if(this._qrt) this._qrt.manage(); }

    send(packet) {
        this._qrt.send(packet);
        return true;
    }
    ...
    */
    close(error=null) {
        ...
        Timer.Clear(this._qrtTimer);
        this._connection = this._qrt = null;
        if(this._negotiate)
    ...
    setSDP(type, sdp) { return new Promise((onSuccess, onFail) => {
        ...
        if(!this._channel && type==SDP.TYPE_OFFER)
            this._connection.ondatachannel({channel: this._connection.createDataChannel("data", {maxRetransmits:0, ordered:false})});
